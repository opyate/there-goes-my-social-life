...
//        findLocationsNear(customCentre); // search for "default" coords
//    } else {
...
//
//                findLocationsNear(results[0].geometry.location);
//
//            } else {
...
function search(query) {
    if ( typeof google == "undefined" ) {
        ...
        //alert("Google is not defined. Searching restos instead");
        ...
        if (query) {
            ...
            //alert("query should not be gotten from the page");
            query = document.getElementById("query").value;
        }

        geocoder.geocode({ 'address': query}, function(results, status) {
            if (status == google.maps.GeocoderStatus.OK) {
                findLocationsNear(results[0].geometry.location, query);
            } else {
                //  search Boffin
                searchRestaurants(query);
            }
...
function searchRestaurants(query) {
    ...
    if (query) {
        //alert("Searching query: " + encodeURI(query));
        ...
        callAPI(searchUrl, query, true);
    }


}

function findLocationsNear(location, query) {
    ...
    callAPI(searchUrl, query, false);
}

function callAPI(searchUrl, query, centerMap) {
    downloadUrl(searchUrl, function(data) {
        ...
        if (markers.length == 0) {
            //alert("zero results, calling searchResults");
            searchRestaurants(query);
            return;
        }
...
function createSidebarEntry(marker, name, address, distance, description, id) {
    ...
    if ( !(typeof google == "undefined") ) {
        ...
        //alert("Google not available, but setting sidebar entry anyway.");

        // TODO when these divs are clicked, we need to add a 'proceed' button
